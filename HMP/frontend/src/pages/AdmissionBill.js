...
import { generateBillPDF } from '../components/PDFGenerator'; 
  ...
  useEffect(() => {
    const fetchBills = async () => {
      try {
        ...
        console.error(error);
        showErrorToast('Error fetching bills.');
      }
    };
  ...
  const handlePay = async (billId, type) => {
    try {
      if (healthCard.topUpAmount <= 0) {
        showErrorToast('Insufficient points. Please top up some points.');
        return;
      }
      ...
      showErrorToast(error.response?.data?.message || 'Payment failed. Please top up your health card.');
    }
  };
